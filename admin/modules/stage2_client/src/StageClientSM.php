...
class StageClientSM {
  ...
  */
  ...
  public static function stage2_get_varspat($language,$var_tree_id,$unpublished){
    ...
    foreach ($query as $key => $value) {
      ...
      $exists = db_query("select * from s2.spatial_layer_date where spatial_layer_id = ".$value->spatial_layer_id." and valid_from <= '".$value->valid_from."';")->fetchField();
      if(!$exists){
        continue;
      }
  ...
  public static function stage2_get_varval($var_values_id,$prop,$lang,$join=true,$innerJoin=false){
    ...
    $q->addField('sld','spatial_layer_id','slid');
    $q->addField('sld','borders');
    $q->addField('sld','table_name');
    $q->addExpression("'{$ds->var_values_valid_from}'-sld.valid_from", "diff");
    $q->addExpression("min('{$ds->var_values_valid_from}'-sld.valid_from) over (partition by sld.spatial_layer_id)",'min_diff');

    $q=db_select($q,'q');
    $q->fields('q',['slid','borders','table_name']);
    $q->where('diff = min_diff');

    if ($lang !='en'){
      $q->join('s2.translations','translations','q.slid = translations.orig_id');
      ...
      $q->addField('translations', 'translation', 'name');
    }
    else {
      $q->join('s2.spatial_layer','sl','q.slid = sl.id');
      $q->addField('sl','name','name');
      $q->addField('sl','weight','weight');
    }
  ...
  public static function getFileUrl($format,$var_values_id,$var_values_valid_from,$spatial_layer_date_id){
    ...
    if (!in_array($format,['SHAPE-ZIP','CSV'])){
      throw new \Exception(t('Only SHAPE-ZIP and CSV formats are supported'));
    }
    ...
    $propertyName='';
    if (!empty($var_values_id) || $format=='CSV'){
      $geom='geom,';
      if ($format=='CSV') $geom='';
    ...
    $conn=db_query("SELECT value from s2.advanced_settings where setting='gsrv'")->fetchField();
    $conn=json_decode($conn);
	return "{$conn->protocol}://{$conn->hostname}:{$conn->port}/{$conn->path}/stage/ows?service=WFS&version=1.0.0&request=GetFeature$propertyName&typeName=stage:$view_name&outputFormat=$format&format_options=CHARSET:UTF-8";
  }
  ...
  */
  ...
  public static function shp2export($var_values_id, $all_variables){
	 $ds=self::getValuesDataSet($var_values_id);

		 // get url for zip
		 if(!intval($all_variables)){
			$fileUrl = self::getFileUrl('SHAPE-ZIP',$var_values_id,null,null);
		 }else{
			$fileUrl = self::getFileUrl('SHAPE-ZIP',0,$ds->var_values_valid_from,$ds->spatial_layer_date_id);
		 }
...
public static function tsv2export($var_values_id, $all_variables){
	$columns = ['__gid_'];
	$ds=self::getValuesDataSet($var_values_id);
    $var_values_valid_from=$ds->var_values_valid_from;
    $spatial_layer_date_id=$ds->spatial_layer_date_id;
	...
	// return [];
		 ...
		 if ($zip->open("sites/default/files/$tableName.zip",\ZipArchive::CREATE) === TRUE) {
			$zip->addFile("sites/default/files/$tableName.tsv","data.tsv");
      ...
      $zip->addFile("sites/default/files/$tableName.html","info.html");
		...
		unlink("sites/default/files/$tableName.tsv");
	}
...
public static function stage2_client_get_var_img($var_tree_id){
  ...
  return ($result->picture);
	...
	public static function stage2_client_get_varpropdesc($var_values_id,$lang){
		...
		$var_names_id = db_query("SELECT var_names_id FROM s2.var_values where published = 1 and id =".$var_values_id.";")->fetchField();
		$valid_from = db_query("SELECT   EXTRACT(Year FROM valid_from) FROM s2.var_values where published = 1 and id =".$var_values_id.";")->fetchField();
		$var_values_id = db_query("SELECT vv.id, sl.tsuv, sl.id as slid
									FROM s2.var_values vv
									JOIN s2.spatial_layer sl on vv.spatial_layer_id = sl.id
									where var_names_id =".$var_names_id." and EXTRACT(Year FROM valid_from) = ". $valid_from." order by sl.weight desc;")->fetch();

		$return['vs_lay'] = db_query("SELECT name from s2.spatial_layer WHERE id =".$var_values_id->slid)->fetchField();

		if ($var_values_id && $var_values_id->tsuv){
			$ds=self::getValuesDataSet($var_values_id->id);
			$valueMD=StageFormsCommon::getValueMD($ds);
		...
		if ($lang=='en'){
			...
			$que->condition('trans.orig_id',$var_values_id->slid);
...
// SELECT * from table WHERE column SIMILAR TO '(AAA|BBB|CCC)%';
    ...
    $sel_var = db_query("SELECT * FROM s2.var_values AS vv WHERE vv.id = $var_values_id;")->fetch();

    // get ids of all variables that meet condition
    $que1 = db_query("SELECT id FROM s2.var_values AS vv
      WHERE vv.published = 1
      AND vv.spatial_layer_id = $sel_var->spatial_layer_id
      AND vv.var_names_id = $sel_var->var_names_id
      ORDER BY vv.valid_from;")->fetchAll();
  ...
  */
    ...
    // do spatial query
		...
		$return = array();
		foreach($ds2 as $row){
			array_push($return, floatval($row[0]));
		}
    ...
    $formula = db_query(
      ...
      if (strlen(preg_replace('/\s+/', '', $delineation)) == 0){
        ...
        foreach ($split_formula[1] as $key => $value) {
          ...
          if ($value <> 'area'){
            $nid = db_query("SELECT id FROM s2.var_names where short_name = '".$value."'")->fetch()->id;
