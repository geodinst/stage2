  ...
  */
  ...
  // delete entries from s2.spatial_layer_date table
     ...
     try {
      db_query("delete from s2.var_values where var_names_id=? and spatial_layer_id=? and valid_from=?",[$variable['var_names_id'],$variable['spatial_layer_id'],$variable['valid_from']]);
 		  $return_value = db_insert('s2.var_values')
 			->fields($variable)
 			->execute();
			$varid = StageDatabase::lastInsertedId('id', 's2.var_values');

			self::stage_inspire('save',$varid,$variable,$tree_menu);
 		}
 		catch (\Exception $e) {
 		  $return_value = drupal_set_message('db_insert failed. Message = %message, query= %query', array(
 			'%message' => $e->getMessage(),
 			'%query' => $e->query_string,
 		  )
 		  , 'error');
   ...
   public static function stageResetClientLabels(){
    db_delete('s2.var_labels') ->execute();
   ...
   /**** Log *****/
   public static function stageLog($action,$report){
   ...
   */
   ...
   public static function deleteVariables($entry = array()) {
     ...
     foreach($entry as $row){
       ...
       db_delete('s2.var_values')
       ->condition('var_names_id', $var_names_id)
       ->condition('spatial_layer_id', $spatial_layer_id)
       ->execute();
     }
   }
   ...
   public static function stage_reset_advanced_settings(){
     // deleta all from advanced settings table
      ...
      db_insert('s2.advanced_settings')
              ->fields(array(
               'setting'=>   'gsrv',
               'value'=> json_encode(array('port'=>8081,
                                           'hostname'=>'localhost',
                                           'protocol'=>'http',
                                           'path'=>'geoserver',
                                           'username'=>'admin',
                                           'password'=>'geoserver'),JSON_UNESCAPED_UNICODE),
