...
****/
   ...
   */
	public static function addCoordinateSpatialUnit($entry) {
		$return_value = NULL;
		try {
		  $return_value = db_insert('s2.spatial_layer')
			->fields($entry)
			->execute();
		}
		catch (\Exception $e) {
...
****/
   ...
   */
	public static function updateVariableProperties($entry = ""){
		// get saved properties
		$exists = self::getVariableProperties();
		$id = -1;
   ...
   */
	public static function insertSpecialValues($id, $data,$inputs){
		// delete all with same properties id
		db_delete('s2.special_values')
			->condition('var_properties_id', $id)
			->execute();

		// save special values data
		$i=0;
		$entry = array();
		foreach($data as $value){
			$entry[] = array('var_properties_id' => $id,
								'special_value' => $value['value'],
								'legend_caption' => array_values($inputs['lc'])[$i],
								'color' => array_values($inputs['col'])[$i]
							);
							$i++;
		}
		...
		foreach($entry as $row){
			try {
			  $return_value = db_insert('s2.special_values')
				->fields($row)
				->execute();
			}
			catch (\Exception $e) {
   ...
   */
	...
	public static function getSpecialValues($id){
		...
		$select->fields('sv', array('special_value'))
