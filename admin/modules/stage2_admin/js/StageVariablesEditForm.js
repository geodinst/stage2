(function ($) {
  ...
  function updateUI(){
    if (isManual()) return;
    var cm=parseInt($('[data-drupal-selector="edit-manual-parameters-auto-classification-interval"]').find(':selected').val());
    if (cm==8) {
      $('#cb').hide();
      $('#less_more').show();
    }
  ...
  function manualLegendSettings(refreshClassBreaks){
    var legendSettings={cp:$('#colorBrewerInput').val(),
                   cb: -1,
                   cm: 0,
                   decimalPlaces:parseInt( $('[data-drupal-selector="edit-manual-parameters-decimals"]').find(':selected').val())
            };
  ...
  Drupal.behaviors.stage2_admin = {
    attach: function (context, settings) {
      ...
      if (drupalSettings.variableValues && ($(context).data('drupal-selector')=='stage-geospatial-layers-edit-form')){
        ...
        var $specialValuesTable=$("[data-drupal-selector='edit-manual-parameters-special-values-special-values-table'] tr");
        var specialValues=[];
        ...
        $('[data-drupal-selector="edit-manual-parameters-auto-classification-interval"]').once().change(function(){
          updateUI();
        });
      }
