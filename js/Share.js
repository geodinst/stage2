	...
	return function() {
		...
		var rendering=false;
		 ...
		 function populate_export_image_popup(description_var) {
			...
			content_export += '<textarea class = "ta_export" id="set_map_title" maxlength="30" rows="2" WRAP="HARD">'+title+'</textarea><br>';
		...
		function render_preview(labels, scale_length) {
			...
			imageObj.xOffset=xOffset;
			imageObj.yOffset=yOffset;

			imageObj.onload = function() {
				var canvas = document.createElement("canvas");
				canvas.width = this.width+this.xOffset;
				canvas.height = this.height+this.yOffset;
				...
				ctx.drawImage(canvas, x_sep_line_length + canvas_width * inner_content_padding , y_padding_logo_surs);
				rendering=false;
			};
